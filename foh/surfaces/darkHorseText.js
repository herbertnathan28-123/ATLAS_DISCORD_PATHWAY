...
function _compactDarkHorseAdvice(viewModel) {
  ...
  if (action) keep.push(_truncate(action, 118));
  if (confirmation) keep.push(_truncate(confirmation, 104));
  if (dollars) keep.push(_truncate(dollars, 104));
...
function _leadSymbol(viewModel, opts) {
  if (opts && opts.symbol) return String(opts.symbol).toUpperCase();
  const eventMatch = String(viewModel.EVENT_DAY_REFERENCE || '').match(/^Event:\s*([A-Z0-9]{2,12})\b/im);
  if (eventMatch) return eventMatch[1].toUpperCase();
  const summaryMatch = String(viewModel.BRIEFING_SUMMARY || '').match(/\bLead:\s*([A-Z0-9]{2,12})\b/i);
