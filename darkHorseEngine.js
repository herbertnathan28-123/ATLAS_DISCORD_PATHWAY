...
function buildDarkHorseDegradedSummary(ranking, volatility, reason, opts) {
  ...
  if (!top.length) {
    return [
      ...
      '',
      '🟪 NEXT REVIEW',
      'Next scheduled Dark Horse scan: ' + nextReview,
      '',
  ...
  const standoutLines = top.length ? top.map((c, idx) => {
    ...
    return [
      ...
      '   CURRENT ADVICE — AT RELEASE: Monitor only until rendered card recovers; no entry without confirmation.',
  ...
  return [
    ...
    standoutLines,
    '',
    'BUILDING / PRE-RADAR: degraded renderer; scan state remains live and re-checks the same candidate set next cycle.',
    'WHAT WOULD PROMOTE A CANDIDATE NEXT: rendered card recovers and candidate keeps score, structure, confirmation, and macro/Jane alignment.',
    'WHAT CANCELS THE WATCH: confirmation fails, invalidation breaks, volatility conflicts, or candidate drops from the next scan.',
    ...
    '',
    'SOURCE / ENGINE STATUS: Source: ATLAS scan engine · freshness: LIVE · renderer fallback reason: ' + (reason || 'unknown'),
    '',
    DH_HARD_BOUNDARY,
