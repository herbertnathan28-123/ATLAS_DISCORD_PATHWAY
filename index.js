...
let _renderPane=0;
let _renderTF='1D';
async function pane_focus(i){_renderPane=i;}
async function chart_set_timeframe(tf){_renderTF=tf;}
async function capture(){return fetchChartImage(_renderSymbol,_renderTF);}
...
async function renderAllPanels(symbol){
  ...
  const htfTF=['1W','1D','240','60'];
  const ltfTF=['30','15','5','1'];
  const htfP=[];
  for(let i=0;i<4;i++){await pane_focus(i);await chart_set_timeframe(htfTF[i]);await wait(150);htfP.push(await capture());}
  const ltfP=[];
  for(let i=0;i<4;i++){await pane_focus(i);await chart_set_timeframe(ltfTF[i]);await wait(150);ltfP.push(await capture());}
