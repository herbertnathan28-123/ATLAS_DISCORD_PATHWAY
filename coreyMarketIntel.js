...
function _compactMarketIntelFallback(content, reportId) {
  ...
  const body = keep.join('\n') || [
    ...
    MI_HARD_BOUNDARY,
    '🔴 THE CALL',
    'Monitoring only until the renderer recovers.',
    '',
    '🔵 MARKET IMPACT',
    'Market impact is degraded this cycle; use calendar timing only until the next refresh restores the rendered packet.',
    '',
    '🔵 SOURCE NOTE',
    'Source: ATLAS runtime · freshness: degraded fallback',
    MI_HARD_BOUNDARY,
    '✅ END OF MARKET INTEL REPORT',
    'Report ID: ' + (reportId || 'MI-pending'),
    MI_HARD_BOUNDARY,
