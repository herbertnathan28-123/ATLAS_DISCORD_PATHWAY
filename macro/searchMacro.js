...
function formatSearchResponse(ctx) {
  ...
  lines.push('');
  lines.push('🟦 BEFORE');
  lines.push('Confirm the scheduled event, source state, and mapped affected markets before adding new risk.');
  lines.push('');
  lines.push('🟧 DURING');
  lines.push('Wait through the release window; do not treat the first impulse as valid until Jane and structure agree.');
  lines.push('');
  lines.push('🟩 AFTER');
  lines.push('Reassess after the first confirmed close, then use the confirmation/degradation notes below.');
  lines.push('');
  lines.push('🧬 COREY CLONE STATUS');
  lines.push('Corey Clone: ' + ctx.cloneSummary.status + ' · historical gate=' + (ctx.cloneSummary.usableForDecision ? 'decision-grade' : 'not decision-grade'));
  ...
  lines.push('Source: ' + (source.source || 'none') + ' · mode: ' + (source.mode || 'none') + ' · confidence: ' + (packet.confidenceScore || 'pending'));
  lines.push('Degradation: ' + (ctx.degradationReason === 'none' ? 'none reported' : userFacingText(ctx.degradationReason)));
  lines.push('');
  lines.push('✅ VALIDITY');
  lines.push('Validity: monitoring only until Jane, Corey Clone gate, source state, and structure confirmation support the same path.');
  lines.push('');
  lines.push(MACRO_HARD_BOUNDARY);
