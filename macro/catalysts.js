...
function build(input) {
  ...
  // Bold-label list — readable on mobile Discord (pipe tables render
  // poorly on small screens). One block per event / cluster.
  for (const r of rows) {
    if (r.kind === 'single') {
      const e = r.item;
      ...
      lines.push(`**${e.title || '—'}** (${e.currency || '—'}) — ${e.impact || '—'} impact`);
      lines.push(`  · When: ${t} UTC  · Forecast: ${f}  · Previous: ${p}`);
    } else {
      // Inflation cluster — one block, comma-joined event names + earliest time.
      ...
      lines.push(`**${r.items.length} ${r.currency || ''} inflation prints** (${titles})`);
      lines.push(`  · First print: ${t} UTC+  · Impact: ${impacts}`);
    }
    lines.push('');
  }
  ...
  for (const r of rows) {
    if (r.kind === 'inflation') {
      ...
      lines.push('Action rule: Entry not probable for this validity window inside ±2h of any print in this cluster. After the LAST print, wait for fresh primary-timeframe structure to reform before re-engaging.');
      lines.push('');
    } else {
      const e = r.item;
      lines.push(`**${e.title} (${e.currency || '—'})** — ${e.impact || 'standard'} impact. Action: Entry not probable for this validity window inside ±2h of release; resume after fresh primary-timeframe structure prints post-release.`);
      lines.push('');
    }
  }
