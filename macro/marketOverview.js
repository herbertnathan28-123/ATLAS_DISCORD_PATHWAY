...
function build(input) {
  ...
  lines.push(`**US Dollar Strength (DXY):** tracked via UUP ETF — ${fmpDxy?.price != null ? '$' + fmpDxy.price : ctx?.dxy?.price != null ? '$' + ctx.dxy.price.toFixed(2) : 'n/a'}, score ${score(dxyScore)}, bias ${dxyBias}. The lower the USD pushes, the more upside fuel for non-USD majors and metals; the higher it pushes, the heavier the squeeze on EURUSD / GBPUSD and the bid in USDJPY. ${arrow(-dxyScore)}`);
  lines.push('');
