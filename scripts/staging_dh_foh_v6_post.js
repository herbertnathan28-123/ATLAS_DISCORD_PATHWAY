...
(async function main() {
const rendered = await foh.renderChartCardAttachments(sanitised.messages);
...
console.log('chart PNG files: ' + rendered.chartCardCount);
...
if (!POST_MODE) {
  for (let i = 0; i < messages.length; i++) {
    ...
    if (Array.isArray(messages[i].files) && messages[i].files.length) {
      body.files = messages[i].files.map(f => ({ name: f.name, contentType: f.contentType, bytes: f.data ? f.data.length : 0 }));
    }
...
await (async function postChain() {
