...
//         "── NEW ──" text.
//   B13 — Transport: emits Discord-renderable messages only.
//         Chart-card PNG attachment is a future lane (L5); the
//         embed ships without an image when no renderer is wired
//         (graceful degrade per matrix B13.c — no placeholder
//         image).
...
//   - No scoring / thresholds / scheduler / transport edits.
...
function _buildingAndReferenceMessage(opts, urlMap) {
  ...
  const embed = {
    ...
    fields: [
      ...
      { name: 'Rendered ATLAS chart cards — next evolution', value: 'Future scans will replace this prototype with a snapshot of the actual candidate, with the four zones drawn on live price action.', inline: false },
    ],
    footer: { text: 'ATLAS · Chart reference · prototype frame — to be replaced with live-snapshot chart cards' },
  };
